...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIM2) {
		for (int i = 0; i < LED_COUNT; i++) {
			volatile BlinkingLedConfig *conf = &ledConfigs[i];
			conf->counterMs++;
			if (conf->counterMs >= conf->toggleThresholdMs) {
				conf->shouldToggle = true;
				conf->counterMs = 0;
			}
		}
	}
...
//  HAL_TIM_Base_Start_IT(&htim2);
  ...
  while (1)
  {
	for (int i = 0; i < LED_COUNT; i++) {
		volatile BlinkingLedConfig *conf = &ledConfigs[i];
		if (conf->shouldToggle) {
			HAL_GPIO_TogglePin(conf->port, conf->pin);
			conf->shouldToggle = false;
		}
	}
//	  debugTimerCounter = __HAL_TIM_GET_COUNTER(&htim2);
